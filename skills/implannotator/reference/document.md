...
- The user just ran `$implannotator init` and needs the visual side documented.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. `$implannotator document --seed` forces seed mode regardless of code presence.
...
### Step 4b: Write .implannotator/design.json sidecar (extensions only)

The frontmatter owns token primitives (colors, typography, rounded, spacing, components). The sidecar at `.implannotator/design.json` carries **what Stitch's schema can't hold**: tonal ramps per color, shadow/elevation tokens, motion tokens, breakpoints, full component HTML/CSS snippets (the panel renders these into a shadow DOM), and narrative (north star, rules, do's/don'ts). It extends the frontmatter, it doesn't duplicate it.

Regenerate the sidecar whenever you regenerate root `DESIGN.md`. If the user only asks to refresh the sidecar (e.g., from the live panel's stale-hint), preserve `DESIGN.md` and write only `.implannotator/design.json`.
...
If the project has **no component library yet** (bare landing page, new project), synthesize canonical primitives from the tokens using best-practice defaults consistent with the DESIGN.md's rules. Every `.implannotator/design.json` has *something* to render, even on day zero.
...
2. Mention that `.implannotator/design.json` was also written alongside; the live panel will now render this project's actual button/input/nav primitives instead of generic approximations.
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `$implannotator document` once there's code, to capture the real tokens and components. OK?"
...
```markdown
<!-- SEED: re-run $implannotator document once there's code to capture the actual tokens and components. -->
```
...
Seed mode writes a minimal frontmatter with `name` and `description` only; no colors, typography, rounded, spacing, or components yet. Real tokens land on the next Scan-mode run. Skip the `.implannotator/design.json` sidecar in seed mode for the same reason: nothing to render.

### Step 4: Confirm

1. Show the seed DESIGN.md. Call out that it is a seed (the marker is the literal commitment).
2. Tell the user: "Re-run `$implannotator document` once you have some code. That pass will extract real tokens and generate the sidecar."
