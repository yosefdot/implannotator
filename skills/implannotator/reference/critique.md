...
2. **Compute the slug**:
   ```bash
   node <implannotator-skill-directory>/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
   Keep it. If the command exits non-zero, skip persistence and trend for this run, but continue the critique.
3. **Read `.implannotator/critique/ignore.md`** if it exists. Drop matching findings silently; it is the only prior-run input critique consumes.
...
- If `spawn_agent` is exposed but the user did not explicitly allow sub-agents, ask exactly once: "Implannotator critique is designed to run two independent sub-agents for an unanchored assessment. May I use sub-agents for this critique?" Then stop until the user answers.
...
Evaluate:
- **AI slop**: Would someone believe "AI made this" immediately? Check all DON'T guidance from the parent Implannotator skill.
...
CLI scan:
```bash
node <implannotator-skill-directory>/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node <implannotator-skill-directory>/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `implannotator` console messages, then stop the live server.
5. For multi-view targets, inject on 3-5 representative pages.

Codex Browser note: Use the Browser skill. Do not spend a Browser attempt on `file://`. Only call `visibility.set(true)` after mutable script injection is confirmed for the `[Human]` overlay path; verify with `get()`. Use `tab.dev.logs({ filter: "implannotator" })` for console results. Its Playwright `evaluate(...)` surface is read-only; do not rely on it for mutation.
...
Codex final-answer note: `$implannotator critique` produces a report artifact, so the final chat response should intentionally exceed the usual concise close-out style. Do not title the final response "Critique Summary" unless the user explicitly asked for a summary.
...
- **Suggested command**: Which command could address this (from: $implannotator adapt, $implannotator animate, $implannotator audit, $implannotator bolder, $implannotator clarify, $implannotator colorize, $implannotator critique, $implannotator delight, $implannotator distill, $implannotator document, $implannotator harden, $implannotator layout, $implannotator onboard, $implannotator optimize, $implannotator overdrive, $implannotator polish, $implannotator quieter, $implannotator shape, $implannotator typeset)

#### Persona Red Flags
> *Consult the [Personas reference](#persona-based-design-testing) below.*

Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `AGENTS.md` contains a `## Design Context` section from `implannotator init`, also generate 1-2 project-specific personas from the audience/brand info.
...
Once the report above is finalized, write it to `.implannotator/critique/` so the user can refer back, and so `$implannotator polish` can pick up the priority issues without a copy-paste.
...
2. **Pass the structured metadata** through `IMPLANNOTATOR_CRITIQUE_META` (JSON), then run the write command:
   ```bash
   IMPLANNOTATOR_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node <implannotator-skill-directory>/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node <implannotator-skill-directory>/scripts/critique-storage.mjs trend <slug> 5
   ```
...
5. **Append a single line to the user-visible output**, after the report and before the questions:
   ...
   > Wrote `.implannotator/critique/<filename>`.
...
- Only recommend commands from: $implannotator adapt, $implannotator animate, $implannotator audit, $implannotator bolder, $implannotator clarify, $implannotator colorize, $implannotator critique, $implannotator delight, $implannotator distill, $implannotator document, $implannotator harden, $implannotator layout, $implannotator onboard, $implannotator optimize, $implannotator overdrive, $implannotator polish, $implannotator quieter, $implannotator shape, $implannotator typeset
...
- End with `$implannotator polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `$implannotator critique` after fixes to see your score improve.
...
If `AGENTS.md` contains a `## Design Context` section (generated by `implannotator init`), derive 1–2 additional personas from the audience and brand information:
