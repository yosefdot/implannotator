...
**Output**: A design brief that can be handed off to $implannotator craft, or directly to $implannotator for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Based on the brief, list which implannotator reference files would be most valuable during implementation (e.g., layout.md for complex layouts, animate.md for animated features, interaction-design.md for form-heavy features, typeset.md for typography-driven pages, colorize.md for color-led brands).
...
Once confirmed, the brief is complete. The user can now hand it to $implannotator, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use $implannotator craft instead, which runs this command internally.)
