...
The global bar **Implannotator mark** dims and shows a pulsing amber dot when no agent is long-polling `/poll`. Hover the mark for the hint; restart `live-poll.mjs` to reconnect.
...
```bash
node <implannotator-skill-directory>/scripts/live.mjs
```
...
```
LOOP:
  node <implannotator-skill-directory>/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node <implannotator-skill-directory>/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
The live helper persists an append-only journal under `.implannotator/live/sessions/`. Browser checkpoints are advisory but durable; the journal is canonical. This is local durable recovery state, not project source.

Use these commands when the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node <implannotator-skill-directory>/scripts/live-status.mjs
node <implannotator-skill-directory>/scripts/live-resume.mjs --id SESSION_ID
node <implannotator-skill-directory>/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node <implannotator-skill-directory>/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
The scaffold has **no** `data-implannotator-variant="original"`. Variants are net-new HTML+CSS inserted at `insertLine`. Load `brand.md` or `product.md` (freeform only, no action sub-command). Write all variants in one edit, then `--reply done`.

For Svelte/SvelteKit targets, `live-insert.mjs` returns `previewMode: "svelte-component"` with `mode: "insert"`, `file` pointing at a temporary `node_modules/.implannotator-live/<id>/manifest.json`, `componentDir` pointing at the variant component files, and `sourceFile` pointing at the real `.svelte` route. Write each inserted variant as a real Svelte component (`v1.svelte`, `v2.svelte`, …) under `componentDir`. Insert variants must be non-empty net-new content with a single top-level root, no `data-implannotator-*` attributes, and CSS in each component's `<style>` block. Do **not** edit the route source during generation; the browser mounts the temporary component before/after the live anchor while the user cycles variants. On Accept, `live-accept.mjs` inserts the selected component markup into `sourceFile` immediately and deletes the temp session after the source write succeeds.
...
```bash
node <implannotator-skill-directory>/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
- `scoped`: use `@scope ([data-implannotator-variant="N"])` rules.
- `astro-global-prefixed`: use explicit `[data-implannotator-variant="N"]` selector prefixes and the exact `styleTag` returned by the tool.
...
<style data-implannotator-css="SESSION_ID">
...
<div data-implannotator-variant="2" style="display: none">
  <!-- variant 2: full element replacement -->
</div>
<div data-implannotator-variant="3" style="display: none">
...
For `styleMode: "scoped"`, author every `:scope` rule with a descendant combinator. The `@scope` boundary is the **variant wrapper `<div data-implannotator-variant="N">`**, not the element you're designing. A bare `:scope { background: cream; }` styles the wrapper, not the inner replacement, so the cream lands on a `display: contents` shell while the actual element keeps page defaults. Always step in: `:scope > .card`, `:scope > section`, `:scope .hero-title`, etc. The fake test agent's CSS in `tests/live-e2e/agent.mjs` is a faithful template; every scoped rule starts `:scope > ...`.

**JSX / TSX target files.** Wrap `<style>` content in a template literal so the CSS `{` / `}` aren't parsed as JSX expressions, and use `className=` / `style={{…}}` on every variant element. Keep `data-implannotator-*` attributes as-is; they're plain strings:

```tsx
<style data-implannotator-css="SESSION_ID">{`
  @scope ([data-implannotator-variant="1"]) { ... }
  @scope ([data-implannotator-variant="2"]) { ... }
`}</style>
<div data-implannotator-variant="1">
  {/* variant 1 */}
</div>
...
The wrap script already gives you a single-rooted JSX wrapper: a `<div data-implannotator-variants="…">` outer element with the marker comments tucked inside. Drop the variants block above into the "Variants: insert below this line" comment and the source stays valid TSX.
...
**Freeform (`action` is `implannotator`) bias.** You did not load a sub-command reference, so you must **choose** signature axes yourself. Match the budget table: for a hero or large composition, that means **2–3 axes per variant**, not 1. Prefer knobs that sit on the dimensions where your three variants actually differ (if density varies, expose it as a `steps` knob; if color commitment varies, expose it as a `range`). A hero that ships with **0** params is almost always a mistake, not a judgment call. A hero with exactly **1** param is underweight unless the design is genuinely a fixed-point comparison. Start from the budget table, not from zero.
...
```html
<div data-implannotator-variant="1" data-implannotator-params='[
...
**Signature params per action.** For named sub-commands, read that action’s `reference/<action>.md` for one or two **MUST** params (e.g. `layout` → `density`). Those are non-negotiable when the design can express them. **Freeform has no file-level MUST**; the **Freeform (`implannotator`) bias** in this section is the stand-in. If the user’s action is both stylized and sub-command (e.g. `colorize`), the sub-command’s MUST list takes precedence for its axes; still respect the **Hard cap** and add no redundant duplicate knobs.
...
```html
<!-- implannotator-param-values SESSION_ID: {"color-amount":0.7,"density":"packed"} -->
```
...
```bash
node <implannotator-skill-directory>/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <implannotator-skill-directory>/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
1. Manually write the wrapper scaffold into the **served** file (the one the browser actually loaded). Use the same structure `live-wrap.mjs` produces; `<!-- implannotator-variants-start ID --><div data-implannotator-variants="ID" data-implannotator-variant-count="3" style="display: contents">…</div><!-- end -->`.
...
3. **Bake in parameter values while rewriting selectors.** For `@scope ([data-implannotator-variant="N"])` wrappers: retarget to real, semantic classes on the accepted HTML (`.why-visual--v2 .v2-label { … }`). For `:scope[data-p-<id>="VALUE"]` selectors: keep only the branch matching the chosen value from the param-values comment; drop the others (they're dead after accept). For `var(--p-<id>, DEFAULT)` in the CSS: either substitute the literal value, or if the param is still useful as a knob going forward, leave the var and update its initial declaration to the chosen value.
4. **Unwrap the accepted content.** Delete the inner `<div data-implannotator-variant="N" style="display: contents">` that wraps it. On JSX/TSX, also delete the outer `<div data-implannotator-carbonize="SESSION_ID" style={{ display: 'contents' }}>` wrapper if present (accept adds it so ternary/`return` slots keep a single root). Drop `data-implannotator-params` and any `data-p-*` attributes; those are live-mode plumbing, not source.
5. **Delete the inline `<style>` block, the `<!-- implannotator-param-values -->` comment if present, and both `<!-- implannotator-carbonize-start/end -->` markers.** Also drop any `@scope` rules for variants other than the accepted one; those are dead code now.
...
```bash
node <implannotator-skill-directory>/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
When native subagents are available, delegate source edits to `implannotator_manual_edit_applier` / `implannotator-manual-edit-applier`. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The subagent must not poll or reply. If unavailable, apply inline with the same contract.
...
```bash
node <implannotator-skill-directory>/scripts/live-server.mjs stop
```

Stops the HTTP server and runs `live-inject.mjs --remove` to strip `localhost:…/live.js` from the HTML entry. To stop the server but keep the inject tag (for a quick restart), use `stop --keep-inject`. `.implannotator/live/config.json` persists as project config for future sessions.

Then:
- Remove any leftover variant wrappers (search for `implannotator-variants-start` markers).
- Remove any leftover carbonize blocks (search for `implannotator-carbonize-start` markers).

## First-time setup (config missing or invalid)

If `live.mjs` outputs `{ ok: false, error: "config_missing" | "config_invalid", path }`, write the live config at the reported path. By default this is `.implannotator/live/config.json`.
...
```bash
node <implannotator-skill-directory>/scripts/detect-csp.mjs
```
...
- **`null`**: no CSP; skip to writing `.implannotator/live/config.json` with `cspChecked: true`.
...
```ts
// Dev-only allowance so implannotator live mode can load. Guarded by NODE_ENV.
const __implannotatorLiveDev =
  process.env.NODE_ENV === "development" ? ["http://localhost:8400"] : [];
```

**Append `...__implannotatorLiveDev` to the script-src and connect-src directive arrays.** Per-framework specifics:
...
Idempotency: if `__implannotatorLiveDev` already exists in the file, the patch is already applied; skip asking and just mark `cspChecked: true`.
...
```ts
// Dev-only allowance so implannotator live mode can load.
const __implannotatorLiveDev =
  process.env.NODE_ENV === "development" ? " http://localhost:8400" : "";
```

Then in the CSP value string:
- `script-src 'self' 'unsafe-inline'` → `` `script-src 'self' 'unsafe-inline'${__implannotatorLiveDev}` ``
- `connect-src 'self'` → `` `connect-src 'self'${__implannotatorLiveDev}` ``
...
If a user says "no" to the CSP patch at setup time and later complains that live doesn't work: their dev CSP blocks `http://localhost:8400`. Fix: delete `cspChecked` from `.implannotator/live/config.json` and re-run `live.mjs`: setup will ask again.
