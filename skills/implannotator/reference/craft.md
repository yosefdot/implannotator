...
Build a feature with implannotator UX and UI quality: shape the design, land the visual direction, build real production code, inspect and improve in-browser until it meets a high-end studio bar.
...
Run $implannotator shape, passing along whatever feature description the user provided. Shape is **required** for craft; it is what produces a confirmed direction.
...
Based on the design brief's "Recommended References" section, consult the relevant implannotator reference files. At minimum, always consult:
...
If the harness has **native image generation** (currently Codex via `image_gen`), this step is mandatory. **Stop and load [codex.md](codex.md)**. It covers palette generation, mock exploration, the approval loop, mock-fidelity inventory, and asset slicing via the `implannotator_asset_producer` subagent. Follow Steps A-F in that file, then return here for Step 4.
