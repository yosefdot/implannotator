...
import { getLiveDir } from '../lib/implannotator-paths.mjs';
import { readBuffer as readManualEditsBuffer } from './manual-edits-buffer.mjs';

const APPLY_EVENT_HARD_TIMEOUT_MS = Number(process.env.IMPLANNOTATOR_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS || 150_000);
const APPLY_EVENT_SOFT_DEADLINE_MS = Number(process.env.IMPLANNOTATOR_LIVE_APPLY_EVENT_SOFT_DEADLINE_MS || 120_000);
...
export function manualEditApplyChunkSize(env = process.env) {
  const raw = Number(env.IMPLANNOTATOR_LIVE_MANUAL_EDIT_CHUNK_SIZE);
