...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/implannotator/ImplannotatorLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- implannotator-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- implannotator-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import ImplannotatorLiveRoot from '$lib/implannotator/ImplannotatorLiveRoot.svelte';";
...
export function buildSvelteLiveRootComponent(port) {
  ...
  const HOST_ID = 'implannotator-live-root';

  onMount(() => {
    let host = document.querySelector('implannotator-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('implannotator-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.implannotatorLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-implannotator-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.implannotatorLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__IMPLANNOTATOR_LIVE_ADAPTER__ = 'sveltekit';
    window.__IMPLANNOTATOR_LIVE_UI_ROOT__ = root;
    window.__IMPLANNOTATOR_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.implannotatorLiveScript = 'true';
    document.head.appendChild(script);

    return () => {
      script.remove();
      if (window.__IMPLANNOTATOR_LIVE_UI_ROOT__ === root) delete window.__IMPLANNOTATOR_LIVE_UI_ROOT__;
      if (window.__IMPLANNOTATOR_LIVE_CHROME_MOUNT__?.root === root) delete window.__IMPLANNOTATOR_LIVE_CHROME_MOUNT__;
      if (window.__IMPLANNOTATOR_LIVE_ADAPTER__ === 'sveltekit') delete window.__IMPLANNOTATOR_LIVE_ADAPTER__;
    };
  });
</script>
`;
