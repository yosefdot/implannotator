 ...
 *
 * Variants are real .svelte components under node_modules/.implannotator-live/<session-id>/.
...
export const SVELTE_COMPONENT_ROOT = 'node_modules/.implannotator-live';
export const SVELTE_RUNTIME_FILE = `${SVELTE_COMPONENT_ROOT}/__runtime.js`;
export const DEFERRED_ACCEPTS_FILE = '.implannotator/live/deferred-svelte-component-accepts.json';

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.IMPLANNOTATOR_LIVE_SVELTE_COMPONENT || '')) return false;
...
function buildInsertVariantStub(variantNum) {
  return `${buildPropsScript([])}<div class="implannotator-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .implannotator-insert-preview { display: block; }\n</style>\n`;
}
...
function sanitizeAcceptedSvelteCss(cssLines, variantNum, paramValues = null, rootTag = 'div') {
  ...
  if (!/data-implannotator-variant|implannotator-variant-ready/.test(css)) return cssLines;
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (!prelude || !body || /--implannotator-variant-ready\s*:/.test(body)) return;

  if (/^@scope\b/i.test(prelude)) {
    if (/data-implannotator-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--implannotator-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-implannotator-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, 'g');
}
...
function inlineSvelteComponentInsertAccept({
  ...
  if (/\bdata-implannotator-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: 'Accepted Svelte insert variant contains preview-only data-implannotator attributes', ...resultBase };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), 'implannotator-live', key, 'deferred-svelte-component-accepts.json');
}
...
export function buildSvelteComponentCssAuthoring(count) {
  ...
  return {
    ...
    requirements: [
      ...
      'Declare params in componentDir/params.json keyed by variant number (e.g. {"1": [...], "2": [...]}), NOT as a data-implannotator-params attribute.',
      'Do not use @scope or data-implannotator-variant selectors in component files.',
    ...
    forbidden: [
      ...
      'Do not add data-implannotator-* attributes inside component files. Svelte parses { in attribute values as an expression, so data-implannotator-params with JSON breaks the build; use componentDir/params.json instead.',
    ],
