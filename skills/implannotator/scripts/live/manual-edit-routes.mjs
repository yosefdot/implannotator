    ...
    if (p === '/manual-edit-commit' && req.method === 'POST') {
      ...
      (async () => {
        ...
        try {
          ...
          const requestedMode = (envValue.IMPLANNOTATOR_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
          const useChatRoute = requestedMode === 'chat'
            || (requestedMode === 'auto' && chatAgentLikelyActive());
          if (useChatRoute) {
            routedProvider = 'chat';
            const timeoutMs = Number(envValue.IMPLANNOTATOR_LIVE_COPY_AGENT_TIMEOUT_MS || 120000);
