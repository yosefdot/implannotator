/**
 * Shared library for the Implannotator design hook.
 *
...
export const ENVELOPE_PREFIX = '[implannotator@1]';
...
export const HOOK_LOCAL_IGNORE_PATTERNS = Object.freeze([
  '.implannotator/hook.cache.json',
  '.implannotator/hook.pending.json',
  '.implannotator/config.local.json',
]);

const HOOK_IGNORE_MARKER_OPEN = '# implannotator-hook-ignore-start';
const HOOK_IGNORE_MARKER_CLOSE = '# implannotator-hook-ignore-end';
...
export function getConfigPath(cwd) {
  return path.join(cwd, '.implannotator', 'config.json');
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, '.implannotator', 'config.local.json');
}

export function getCachePath(cwd) {
  return path.join(cwd, '.implannotator', 'hook.cache.json');
}

export function getPendingPath(cwd) {
  return path.join(cwd, '.implannotator', 'hook.pending.json');
}
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run /implannotator audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see /implannotator audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see /implannotator audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) => [
    header,
    ...linesArr,
    ...(omitted ? ['... and more (see /implannotator audit).'] : []),
    '',
    footer,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = '... and more (see /implannotator audit).';
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `/implannotator hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.IMPLANNOTATOR_HOOK_HARNESS;
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  let target = env?.IMPLANNOTATOR_HOOK_LOG;
...
// silence-on-clean can set `IMPLANNOTATOR_HOOK_QUIET=1` — runHook checks that
...
export function appendDesignSystemNote(text, scanOptions) {
  ...
  return `${text}\n\n${ENVELOPE_PREFIX} DESIGN.md is newer than .implannotator/design.json. Run /implannotator document to refresh the design-system sidecar.`;
}
...
function directiveFooter(display, opts = {}) {
  const ignoreFileCommand = `/implannotator hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = opts.grouped
    ? 'run `/implannotator hooks ignore-file <path>` for the specific file'
  ...
  return [
    ...
    '',
    `Do not change intentional design just to satisfy the hook. Do not add source comments such as \`implannotator: ignore\`; those pollute the code and do not suppress hook findings. Persist hook ignores only after the user explicitly confirms the finding is intentional. Prefer the narrowest persisted exception: run the exact \`/implannotator hooks ignore-value ... --shared\` command shown next to a value-specific finding. For \`overused-font\`, use \`ignore-value\` for a specific font and use \`/implannotator hooks ignore-rule overused-font --all-values\` only when the user asks to ignore overused fonts generally. For file-specific findings without an ignore-value command, ${fileIgnoreGuidance}; use \`/implannotator hooks ignore-rule <id>\` only when the user asks to suppress the whole non-value-specific rule. Run /implannotator audit for the full pass.`,
  ].join('\n');
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.IMPLANNOTATOR_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.IMPLANNOTATOR_HOOK_DISABLED)) {
    ...
    if (truthy(env.IMPLANNOTATOR_HOOK_QUIET) || config.quiet === true) {
