/**
 * CLI-side reader/writer for the unified `.implannotator` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.implannotator', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.implannotator', 'config.local.json');
}
...
/**
 * Detector filters shared by `npx implannotator detect` and the design hook.
...
const EXCLUDE_OPEN = '# implannotator-config-ignore-start';
const EXCLUDE_CLOSE = '# implannotator-config-ignore-end';
const EXCLUDE_PATTERNS = ['.implannotator/config.local.json'];
