...
export const IMPLANNOTATOR_DIR = '.implannotator';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getImplannotatorDir(cwd = process.cwd()) {
  return path.join(cwd, IMPLANNOTATOR_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd()) {
  return path.join(getImplannotatorDir(cwd), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd()) {
  return path.join(getImplannotatorDir(cwd), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env } = {}) {
  if (env.IMPLANNOTATOR_LIVE_CONFIG && env.IMPLANNOTATOR_LIVE_CONFIG.trim()) {
    const configured = env.IMPLANNOTATOR_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd()) {
  return path.join(cwd, '.implannotator-live.json');
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd()) {
  return path.join(cwd, '.implannotator-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd()) {
  return path.join(getImplannotatorDir(cwd), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd()) {
  return path.join(cwd, '.implannotator-live', 'annotations');
}
