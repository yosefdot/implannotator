...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `implannotator-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('implannotator-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'implannotator-visual-contrast-hide-style';
      style.textContent = [
        '[data-implannotator-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-implannotator-visual-contrast-target][data-implannotator-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-implannotator-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-implannotator-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-implannotator-visual-contrast-target');
          el.removeAttribute('data-implannotator-bgclip-text');
        }
      } catch {
