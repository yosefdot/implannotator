...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.implannotatorAnalyzeVisualContrast !== 'function') return [];
        return window.implannotatorAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.implannotatorCollectVisualContrastCandidates !== 'function') return [];
      return window.implannotatorCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
...
async function detectUrl(url, options = {}) {
  ...
  try {
    ...
    await profileStepAsync(profile, {
      ...
      window.__IMPLANNOTATOR_CONFIG__ = {
        ...(window.__IMPLANNOTATOR_CONFIG__ || {}),
        autoScan: false,
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.implannotatorDetect) return [];
        return window.implannotatorDetect({ decorate: false, serialize: true });
      });
