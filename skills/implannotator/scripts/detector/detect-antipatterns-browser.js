/**
 * Anti-Pattern Browser Detector for Implannotator
 ...
 * Re-scan: window.implannotatorScan()
 */
(function () {
...
const REPEATED_KICKER_SKIP_SELECTOR = [
  ...
  '[class*="breadcrumb" i]',
  '[aria-hidden="true"]',
  '[data-implannotator-allow-kickers]',
].join(',');
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__IMPLANNOTATOR_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip implannotator's own elements
    if (el.closest && el.closest('.implannotator-overlay, .implannotator-label, .implannotator-banner, .implannotator-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.implannotatorExtension === 'true')
    || document.documentElement.dataset.implannotatorExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes implannotator-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .implannotator-overlay:not(.implannotator-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: implannotator-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .implannotator-overlay.implannotator-visible {
      animation-play-state: running;
    }
    .implannotator-overlay.implannotator-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .implannotator-overlay.implannotator-hover .implannotator-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .implannotator-overlay.implannotator-spotlight {
      z-index: 100002 !important;
    }
    .implannotator-overlay.implannotator-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .implannotator-spotlight-backdrop {
      position: fixed;
    ...
    .implannotator-spotlight-backdrop.implannotator-visible {
      opacity: 1;
    }
    .implannotator-hidden .implannotator-overlay${EXTENSION_MODE ? '' : ':not(.implannotator-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'implannotator-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__IMPLANNOTATOR_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('implannotator-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('implannotator-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('implannotator-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._implannotatorOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('implannotator-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._implannotatorOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._implannotatorOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('implannotator-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    if (el._implannotatorOverlay) detachOverlay(el._implannotatorOverlay);
    const hasSlop = findings.some(f => RULE_CATEGORY[f.type || f.id] === 'slop');

    const fixed = isInFixedContext(el);
    const rect = el.getBoundingClientRect();
    const outline = document.createElement('div');
    outline.className = 'implannotator-overlay';
    ...
    label.className = 'implannotator-label';
    ...
    el._implannotatorOverlay = outline;
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('implannotator-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('implannotator-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'implannotator-overlay implannotator-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('implannotator-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('implannotator-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.implannotator-overlay, .implannotator-label, .implannotator-banner, .implannotator-tooltip')) continue;
      if (el.closest('[id^="implannotator-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.implannotator-overlay, .implannotator-label, .implannotator-banner, .implannotator-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[implannotator] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[implannotator] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__IMPLANNOTATOR_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__IMPLANNOTATOR_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip implannotator's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.implannotator-overlay, .implannotator-label, .implannotator-banner, .implannotator-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the implannotator live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip implannotator-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="implannotator-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__IMPLANNOTATOR_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__IMPLANNOTATOR_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'implannotator-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('implannotator-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[implannotator] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('implannotator-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'implannotator-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__IMPLANNOTATOR_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('implannotator-hidden');
        document.body.classList.toggle('implannotator-hidden', visible);
        window.postMessage({ source: 'implannotator-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('implannotator-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('implannotator-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('implannotator-spotlight', isMatch);
              o.classList.toggle('implannotator-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('implannotator-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('implannotator-spotlight');
          o.classList.remove('implannotator-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'implannotator-ready' }, '*');
  } else {
    if (window.__IMPLANNOTATOR_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[implannotator] scan failed', err);
        }
      };
  ...
  window.implannotatorDetect = detect;
  window.implannotatorDetectAsync = detectAsync;
  window.implannotatorScan = scan;
  window.implannotatorScanAsync = scanAsync;
  window.implannotatorCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.implannotatorAnalyzeVisualContrast = analyzeVisualContrast;
  window.implannotatorGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
