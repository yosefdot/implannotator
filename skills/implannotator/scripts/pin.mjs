 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /implannotator audit.
...
const PIN_MARKER = '<!-- implannotator-pinned-skill -->';
...
/**
 * Find harness skill directories that have an implannotator skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have implannotator installed
    const implannotatorDir = join(skillsDir, 'implannotator');
    if (existsSync(implannotatorDir) || existsSync(join(skillsDir, 'i-implannotator'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /implannotator ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}implannotator ${command}\`.

Invoke {{command_prefix}}implannotator ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with implannotator installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /implannotator ${command} to access it.`);
  } else {
