 ...
 *   3. $IMPLANNOTATOR_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
// can offer `npx implannotator update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.IMPLANNOTATOR_UPDATE_HOST || 'https://implannotator.style').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.IMPLANNOTATOR_UPDATE_CACHE || path.join(os.homedir(), '.implannotator', 'update-check.json');
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  const envDir = process.env.IMPLANNOTATOR_CONTEXT_DIR;
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Implannotator skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Implannotator (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx implannotator update\`." ` +
    `If they agree, run \`npx implannotator update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with IMPLANNOTATOR_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.implannotator', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.IMPLANNOTATOR_NO_UPDATE_CHECK) return null;
