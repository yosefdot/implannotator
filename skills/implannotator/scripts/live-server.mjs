...
import {
  ...
  writeLiveServerInfo,
} from './lib/implannotator-paths.mjs';
...
// DESIGN sidecar is project-local at .implannotator/design.json, with legacy
...
const DEBUG_MANUAL_EDIT_EVENTS = /^(1|true|yes)$/i.test(process.env.IMPLANNOTATOR_LIVE_DEBUG_EVENTS || '');
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, '..', '..', '..', '..', 'cli', 'engine', 'detect-antipatterns-browser.js'),
    path.join(process.cwd(), 'node_modules', 'implannotator', 'cli', 'engine', 'detect-antipatterns-browser.js'),
  ];
  let detectScript = '';
...
function createRequestHandler({ detectScript, liveScriptParts }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .implannotator/design.json
    ...
    //                          - sidecar: .implannotator/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .implannotator/design.json: ' + err.message;
        }
      }
...
function sessionFileMetadataFromPollReply(file) {
  ...
  if (!normalized.includes('node_modules/.implannotator-live/') && !normalized.includes('src/lib/implannotator/')) return base;
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn('[implannotator] Svelte component session cleanup failed:', err.message);
  }
}
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log('[implannotator] applied legacy deferred Svelte component accepts:', JSON.stringify(result));
    }
  } catch (err) {
    console.warn('[implannotator] legacy deferred Svelte component accept apply failed:', err.message);
  }
}
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nImplannotator live server running on ${url}`);
