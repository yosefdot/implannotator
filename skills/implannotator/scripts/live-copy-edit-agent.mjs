...
export function buildCopyEditBatchPrompt(batch, { cwd = process.cwd() } = {}) {
  ...
  return [
    'You are the Implannotator staged copy-edit batch applier.',
    '',
    ...
    '- Never copy browser edit-mode scaffolding into source: no contenteditable, data-implannotator-* markers, wrapper variants, generated style/script tags, or runtime-only attributes.',
    ...
    '- If package.json defines scripts.implannotator:manual-edit-validate, it must pass after edits.',
    '- Check for leftover implannotator-carbonize markers or variant wrapper markers in touched files.',
    '',
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  if (provider === 'mock') {
    const delayMs = Number(env.IMPLANNOTATOR_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0);
  ...
  const outDir = opts.outDir || fs.mkdtempSync(path.join(os.tmpdir(), 'implannotator-copy-batch-'));
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    const markerMatch = findLeftoverImplannotatorMarker(content);
    if (markerMatch) failures.push({ file: relativeFile, reason: 'leftover_implannotator_marker', marker: markerMatch });
...
function findLeftoverImplannotatorMarker(content) {
  const commentMarker = content.match(/^\s*(?:<!--|\{\/\*)\s*implannotator-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*implannotator-variants-(?:start|end)\b/m);
  if (commentMarker) return commentMarker[0];

  const attrPattern = /\bdata-implannotator-(?:variants?|original-text|editable|text-wrap)\s*=/g;
...
function readManualEditValidationScript(cwd) {
  ...
  try {
    ...
    const script = pkg?.scripts?.['implannotator:manual-edit-validate'];
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(/\sdata-implannotator-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g, '')
...
function mockBatchResult(batch, env, cwd = process.cwd()) {
  ...
  const raw = env.IMPLANNOTATOR_LIVE_COPY_AGENT_MOCK_RESULT;
  if (raw) {
    const parsed = parseCopyEditBatchResult(raw);
    if (parsed) return parsed;
    throw new Error('Invalid IMPLANNOTATOR_LIVE_COPY_AGENT_MOCK_RESULT JSON');
  }
  return {
    status: 'done',
...
function applyMockWrites(env, cwd) {
  const raw = env.IMPLANNOTATOR_LIVE_COPY_AGENT_MOCK_WRITES;
  if (!raw) return;
  const writes = tryParseJson(raw);
  if (!writes || typeof writes !== 'object' || Array.isArray(writes)) {
    throw new Error('Invalid IMPLANNOTATOR_LIVE_COPY_AGENT_MOCK_WRITES JSON');
  }
...
export function chooseCopyEditAgent({
  ...
  const mode = (env.IMPLANNOTATOR_LIVE_COPY_AGENT || 'auto').trim().toLowerCase();
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    ...
    '-c', `model_reasoning_effort="${env.IMPLANNOTATOR_LIVE_COPY_AGENT_EFFORT || 'low'}"`,
  ];
  if (env.IMPLANNOTATOR_LIVE_COPY_AGENT_MODEL) {
    args.push('--model', env.IMPLANNOTATOR_LIVE_COPY_AGENT_MODEL);
  }
  args.push('-');
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    lines.push('  • Chat: an Implannotator live session is polling but selection chose another provider — unexpected; please report.');
  } else {
    lines.push('  • Chat: no Implannotator live session is currently polling on this server. Start Implannotator live in your chat to route Apply through the chat agent.');
  }
  lines.push('Fix one of the above, or set IMPLANNOTATOR_LIVE_COPY_AGENT=mock for tests.');
  return lines.join('\n');
}
