 ...
 *
 * The browser Save path stages copy edits in .implannotator/live. This script is
...
function repairAttemptLimit(env = process.env) {
  const value = Number(env.IMPLANNOTATOR_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS || DEFAULT_REPAIR_ATTEMPTS);
...
async function main() {
  ...
  const result = await commitManualEdits({
    ...
    timeoutMs: Number(process.env.IMPLANNOTATOR_LIVE_COPY_AGENT_TIMEOUT_MS || 120000),
  });
