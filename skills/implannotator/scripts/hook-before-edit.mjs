#!/usr/bin/env node
/**
 * Implannotator design hook — Cursor preToolUse write gate.
 *
...
function cursorBlockMessage(findings, filePath, config, cwd) {
  ...
  const blocked = rendered.replace(
    '[implannotator@1] Design hook findings requiring review',
    '[implannotator@1] Implannotator design hook blocked this write before it landed. Design hook findings requiring review',
  );
...
async function main() {
  if (truthy(process.env.IMPLANNOTATOR_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so Implannotator is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow({
      ...audit,
      findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.IMPLANNOTATOR_HOOK_DEBUG) {
    process.stderr.write(`[implannotator-hook-before-edit] ${err}\n`);
  }
