#!/usr/bin/env node
/**
 * `/implannotator hooks <on|off|status|reset>` — manage the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .implannotator/config.json / .implannotator/config.local.json.
 *
 * Usage:
...
const IMPLANNOTATOR_HOOK_COMMAND_MARKERS = [
  'skills/implannotator/scripts/hook-probe.mjs',
  'skills/implannotator/scripts/hook.mjs',
  'skills/implannotator/scripts/hook-before-edit.mjs',
  'skills/implannotator/scripts/hook-after-edit.mjs',
  'skills/implannotator/scripts/hook-stop.mjs',
];
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/implannotator',
    destRel: '.claude/settings.local.json',
    sharedDestRel: '.claude/settings.json',
    manifest: () => ({
      description: 'Implannotator design detector: runs after Edit/Write/MultiEdit on UI files and surfaces findings as system reminders.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/implannotator/scripts/hook.mjs"',
  ...
  {
    provider: '.agents',
    skillRel: '.agents/skills/implannotator',
    destRel: '.codex/hooks.json',
    manifest: () => ({
      description: 'Implannotator design detector: runs after Edit/Write/apply_patch on UI files and surfaces findings as system reminders.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "$(git rev-parse --show-toplevel)/.agents/skills/implannotator/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
  ...
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/implannotator',
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/implannotator/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive a `/implannotator hooks` edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.IMPLANNOTATOR_HOOK_DISABLED;
  const envState = envKill ? `IMPLANNOTATOR_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.implannotator/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.implannotator/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.implannotator/hook.cache.json';
...
function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    if (sharedDest && fileHasImplannotatorHookMarker(sharedDest)) {
      pruneImplannotatorHookFromManifest(dest);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripImplannotatorHookEntries(existingHooks[event]);
...
function fileHasImplannotatorHookMarker(filePath) {
  ...
  return valueHasImplannotatorHookMarker(parsed.hooks);
}

function valueHasImplannotatorHookMarker(value) {
  if (typeof value === 'string') {
    return IMPLANNOTATOR_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasImplannotatorHookMarker);
  if (value && typeof value === 'object') return Object.values(value).some(valueHasImplannotatorHookMarker);
  return false;
}

function stripImplannotatorHookEntry(entry) {
  if (!entry || typeof entry !== 'object') return entry;
  if (valueHasImplannotatorHookMarker(entry.command) || valueHasImplannotatorHookMarker(entry.args)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripImplannotatorHookEntry)
    .filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasImplannotatorHookMarker)) {
    return null;
  }
  return { ...entry, hooks: strippedHooks };
}

function stripImplannotatorHookEntries(entries) {
...
function pruneImplannotatorHookFromManifest(manifestPath) {
  if (!fileHasImplannotatorHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripImplannotatorHookEntries(entries);
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. /implannotator hooks ignore-rule side-tab');
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error('overused-font is value-specific by default. Use /implannotator hooks ignore-value overused-font <font> for a confirmed font, or /implannotator hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.');
  }
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error('Pass a glob, e.g. /implannotator hooks ignore-file "src/legacy/**"');
  const config = mergeDetectorConfig(readRawDetectorConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error('Pass a rule id and value, e.g. /implannotator hooks ignore-value overused-font Inter');
  }
