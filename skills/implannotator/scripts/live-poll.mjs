 ...
 *
 * Usage:
 *   npx implannotator poll                         # Block until browser event, print JSON
 *   npx implannotator poll --stream                # Experimental: keep polling; one JSON line per event
 *   npx implannotator poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   npx implannotator poll --reply <id> done       # Reply "done" to event <id>
 *   npx implannotator poll --reply <id> error "msg" # Reply with error
 */
...
import { readLiveServerInfo } from './lib/implannotator-paths.mjs';
...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with: npx implannotator live');
    process.exit(1);
  }
  return record.info;
}
...
function validateReplyArgs({ id, status }) {
  const usage = "Usage: npx implannotator poll --reply <id> <status> [--file path] [--data '<json>'] [message]";
  if (!id || id.startsWith('--')) {
...
export async function runPollStream(base, token, {
  ...
  process.stderr.write('[implannotator-poll] stream mode: one JSON object per line on stdout; use --reply while this process stays running\n');
...
function handlePollError(err) {
  if (err.code === 'AUTH_FAILED') {
    ...
    console.error('Try restarting: npx implannotator live stop && npx implannotator live');
    process.exit(1);
  }
  if (err.cause?.code === 'ECONNREFUSED') {
    console.error('Live server not running. Start one with: npx implannotator live');
    process.exit(1);
  }
...
export async function pollCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: implannotator poll [options]
...
Harness note:
  ...
  // Reply mode: npx implannotator poll --reply <id> <status> [--file path] [--data '<json>'] [message]
