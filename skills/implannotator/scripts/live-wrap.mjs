 ...
 *
 * Usage:
 *   npx implannotator wrap --id SESSION_ID --count N --query "hero-combined-left" [--file path]
 *
...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: implannotator wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-implannotator-variants="' + id + '" data-implannotator-variant-count="' + count + '" ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' implannotator-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-implannotator-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' implannotator-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' implannotator-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-implannotator-variants="' + id + '" data-implannotator-variant-count="' + count + '" ' + styleContents + '>',
...
function detectStyleMode(filePath) {
  ...
  if (ext === '.astro') {
    return {
      ...
      styleTag: '<style is:inline data-implannotator-css="SESSION_ID">',
    };
  }
  return {
    mode: 'scoped',
    styleTag: '<style data-implannotator-css="SESSION_ID">',
  };
}

function buildCssSelectorPrefixExamples(styleMode, count) {
  if (styleMode !== 'astro-global-prefixed') return [];
  return Array.from({ length: count }, (_, i) => `[data-implannotator-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-implannotator-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-implannotator-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-implannotator-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-implannotator-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-implannotator-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-implannotator-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-implannotator-variant="N"] selector prefixes for this styleMode.',
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-implannotator-variant')) continue;
