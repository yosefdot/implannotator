#!/usr/bin/env node
/**
 * Implannotator design hook — PostToolUse entry point.
 *
...
async function main() {
  ...
  process.env.IMPLANNOTATOR_HOOK_DEPTH = process.env.IMPLANNOTATOR_HOOK_DEPTH || '1';
...
main().catch((err) => {
  ...
  if (process.env.IMPLANNOTATOR_HOOK_DEBUG) {
    process.stderr.write(`[implannotator-hook] ${err}\n`);
  }
  process.exit(0);
});
