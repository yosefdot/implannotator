 ...
 *
 * Each run of /implannotator critique writes a per-target snapshot to
 *   .implannotator/critique/<timestamp>__<slug>.md
 * with a small YAML frontmatter carrying the score + P0/P1 counts.
 *
 * /implannotator polish reads the latest matching snapshot at start as its
...
import { getCritiqueDir } from './lib/implannotator-paths.mjs';
...
export function writeSnapshot({ slug, meta, body, cwd = process.cwd(), now = new Date() }) {
  ...
  // IMPLANNOTATOR_CRITIQUE_META env var) could clobber them, leaving the
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case 'write': {
      ...
      let meta = {};
      const metaArg = process.env.IMPLANNOTATOR_CRITIQUE_META;
      if (metaArg) {
