 ...
 * <div data-implannotator-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' implannotator-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-implannotator-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' implannotator-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' implannotator-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-implannotator-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-implannotator-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-implannotator-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-implannotator-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-implannotator-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-implannotator-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-implannotator-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-implannotator-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'implannotator-variants-start ' + id;
  const endPattern = 'implannotator-variants-end ' + id;
 ...
 * the `<div data-implannotator-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-implannotator-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-implannotator-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('implannotator-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-implannotator-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-implannotator-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-implannotator-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-implannotator-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-implannotator-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-implannotator-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const marker = 'implannotator-variants-start ' + id;
