 ...
 *   1. Check .implannotator/live/config.json (returns config_missing if first-ever run)
 ...
 *   - Open the project's live dev/preview URL in the browser (optional, if browser automation exists)—not `serverPort`; that port is the Implannotator helper for /live.js and /poll
...
Prepare everything for live variant mode in a single command:
  - Checks .implannotator/live/config.json (required, created once per project)
