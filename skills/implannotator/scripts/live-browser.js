/**
 * Implannotator Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__IMPLANNOTATOR_TOKEN__ and window.__IMPLANNOTATOR_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__IMPLANNOTATOR_LIVE_INIT__) return;
  window.__IMPLANNOTATOR_LIVE_INIT__ = true;

  const TOKEN = window.__IMPLANNOTATOR_TOKEN__;
  const PORT = window.__IMPLANNOTATOR_PORT__;
  if (!TOKEN || !PORT) {
    window.__IMPLANNOTATOR_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'implannotator-live';
  const PICK_CURSOR_CLASS = PREFIX + '-pick-cursor';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__IMPLANNOTATOR_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[implannotator] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__IMPLANNOTATOR_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__IMPLANNOTATOR_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__IMPLANNOTATOR_VOCAB__) ? window.__IMPLANNOTATOR_VOCAB__ : [];
  const ICONS = {};
  ...
  let selectedAction = 'implannotator';
  let selectedCount = 3;
  ...
  const domHelpers = window.__IMPLANNOTATOR_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[implannotator] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__IMPLANNOTATOR_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__IMPLANNOTATOR_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__IMPLANNOTATOR_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-implannotator-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-implannotator-placeholder-resize]')?.dataset.implannotatorPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-implannotator-editable], [data-implannotator-original-text], [data-implannotator-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-implannotator-editable')
        || node.hasAttribute('data-implannotator-original-text');
      node.removeAttribute('data-implannotator-editable');
      node.removeAttribute('data-implannotator-original-text');
      node.removeAttribute('data-implannotator-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-implannotator-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.implannotatorPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.implannotatorPlaceholderWidth;
    ...
    placeholder.dataset.implannotatorPlaceholderWidth = 'explicit';
  }
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-implannotator-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-implannotator-variant]:not([data-implannotator-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-implannotator-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.implannotatorMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.implannotatorPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-implannotator-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes implannotator-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: implannotator-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function buildSavingRow() {
    ...
    const spinner = el('div', {
      ...
      animation: 'implannotator-spin 0.6s linear infinite',
      flexShrink: '0',
    });
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[implannotator] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-implannotator-variant="1"
  //        data-implannotator-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-implannotator-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-implannotator-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-implannotator-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[implannotator] Invalid data-implannotator-params JSON:', err.message);
      return [];
    }
  }
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.implannotatorTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.implannotatorTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-implannotator-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.implannotatorEditable = 'true';
      row.el.dataset.implannotatorOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.implannotatorEditable;
      delete row.el.dataset.implannotatorOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.implannotatorOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('implannotator-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('implannotator-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('implannotator-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[implannotator] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes implannotator-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[implannotator] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[implannotator] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[implannotator] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[implannotator] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[implannotator] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[implannotator] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[implannotator] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[implannotator] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.implannotatorTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.implannotatorTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.implannotatorEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-implannotator-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-implannotator-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-implannotator-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-implannotator-variants="' + sessionId + '"]');
    if (!wrapper) return false;
    for (const child of wrapper.children) {
      const v = child.dataset ? child.dataset.implannotatorVariant : null;
      if (!v) continue;
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="implannotator-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('implannotator-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-implannotator-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-implannotator-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-implannotator-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  function loadSvelteRuntime(runtimeModule) {
    const modulePath = runtimeModule || '/src/lib/implannotator/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-implannotator-params`
  ...
  async function applySvelteComponentVariantStyle(variantNum) {
    ...
    if (!scopedCss) return;
    const style = document.createElement('style');
    style.dataset.implannotatorSvelteComponentStyle = sessionId;
    style.dataset.implannotatorVariant = String(variantNum);
  ...
  function scopeCssToSveltePreview(css, sessionId) {
    const prefix = '[data-implannotator-variants="' + String(sessionId).replace(/"/g, '\\"') + '"] ';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[implannotator] Failed to mount Svelte variant ' + variantNum + ' for ' + sessionId + ':', err);
      return false;
    }
  }
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-implannotator-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[implannotator] Could not find original element in live DOM.');
        arrivedVariants = Number(manifest.count) || expectedVariants || 1;
        expectedVariants = arrivedVariants;
      ...
      wrapper.dataset.implannotatorVariants = sessionId;
      wrapper.dataset.implannotatorVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.implannotatorPreview = 'svelte-component';
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.implannotatorComponentMount = sessionId;
      ...
      positionBar();
      saveSession();
      console.log('[implannotator] Mounted ' + arrivedVariants + ' Svelte component variants.');
    } catch (err) {
      console.error('[implannotator] Failed to mount Svelte component variants:', err);
  ...
  function abortSvelteComponentInjection(sessionId, message) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-implannotator-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[implannotator] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- implannotator-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- implannotator-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-implannotator-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[implannotator] Variant wrapper not found in source file.');
          return;
        }
        ...
        const variants = wrapper.querySelectorAll('[data-implannotator-variant]:not([data-implannotator-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.implannotatorVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
        ...
        positionBar();
        saveSession();
        console.log('[implannotator] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[implannotator] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__implannotatorFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-implannotator-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.implannotatorVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-implannotator-variant="' + index + '"]');
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-implannotator-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-implannotator-variants="' + sessionId + '"]') || n.querySelector?.('[data-implannotator-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-implannotator-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-implannotator-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.implannotatorVariants || n.dataset?.implannotatorVariant) {
          ...
          if (n.querySelector?.('[data-implannotator-variants],[data-implannotator-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.implannotatorMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.implannotatorVariantCount || '0');
      ...
      if (arrivedVariants >= expectedVariants && expectedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.implannotatorMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
        refreshParamsPanel();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-implannotator-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No PRODUCT.md found. Variants will be brand-agnostic. Run /implannotator init to generate one.', 7000);
          console.log('[implannotator] Live mode connected.');
        ...
        case 'error':
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[implannotator] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[implannotator] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[implannotator] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[implannotator] Failed to send event:', err);
        throw err;
      }
      console.debug('[implannotator] Dropped optional live event:', err);
      return null;
    }
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.implannotatorOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__IMPLANNOTATOR_LIVE_ADAPTER__ || '').toLowerCase();
    if (adapter === 'svelte' || adapter === 'sveltekit') return true;
    if (currentPreviewMode === 'svelte-component' || svelteComponentSession) return true;
    const wrapper = el?.closest?.('[data-implannotator-variants]');
    return wrapper?.dataset?.implannotatorPreview === 'svelte-component';
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[implannotator] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[implannotator] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[implannotator] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[implannotator] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[implannotator] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[implannotator] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-implannotator-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || acceptWrapper?.dataset?.implannotatorPreview === 'svelte-component';
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-implannotator-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-implannotator-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-implannotator-variants],[data-implannotator-variant],[data-implannotator-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-implannotator-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-implannotator-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-implannotator-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-implannotator-css]');
    if (style && !document.querySelector('style[data-implannotator-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-implannotator-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-implannotator-variants]');
    if (wrapper && wrapper.dataset.implannotatorPreview !== 'svelte-component') return false;
  ...
  function cleanup() {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-implannotator-variants="' + cleanupSessionId + '"]');
      if (wrapper) wrapper.style.display = 'none';
      setTimeout(function() {
        if (!cleanupSessionId) return;
        const lateWrapper = document.querySelector('[data-implannotator-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-implannotator-variant="original"]');
        if (orig) {
  ...
  // If a [data-implannotator-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.implannotatorVariants;
    ...
    // wrapper holds a single mount target, not [data-implannotator-variant] nodes,
    ...
    if (wrapper.dataset.implannotatorPreview === 'svelte-component'
    ...
    if (wrapper.dataset.implannotatorPreview === 'svelte-component') {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.implannotatorVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.implannotatorVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-implannotator-variant]:not([data-implannotator-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[implannotator] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'implannotator-live-pick';
  const INTERACTION_PREFS_KEY = 'implannotator-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'implannotator-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('implannotator-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__IMPLANNOTATOR_STEER_FOCUS_GUARD__) return;
    window.__IMPLANNOTATOR_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('implannotator-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[implannotator.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__IMPLANNOTATOR_STEER_FOCUS_DEBUG__) return;
    window.__IMPLANNOTATOR_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'implannotator-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[implannotator.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[implannotator.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes implannotator-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes implannotator-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes implannotator-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: implannotator-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: implannotator-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Implannotator mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', connected
      ? 'Implannotator live mode'
      : 'Implannotator live mode - agent not polling');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes implannotator-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: implannotator-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Implannotator icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Implannotator live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.implannotator-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'implannotator-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'implannotator-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.implannotatorExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'implannotator-ready') {
      detectReady = true;
    ...
    if (e.data.source === 'implannotator-results') {
  ...
  function teardown() {
    ...
    window.__IMPLANNOTATOR_LIVE_INIT__ = false;
    console.log('[implannotator] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .implannotator/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'implannotator-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .implannotator/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Implannotator-branded. The panel is
  // a viewer of the project's design system, not an Implannotator surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/implannotator document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .implannotator/design.json.</strong> Run <code>/implannotator document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>/implannotator document</code> also generates a <code>.implannotator/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .implannotator/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[implannotator] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[implannotator] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[implannotator] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
