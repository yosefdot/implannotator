 ...
 *
 * On first live run, the agent generates `.implannotator/live/config.json`
...
import { resolveLiveConfigPath } from './lib/implannotator-paths.mjs';
import {
...
const MARKER_OPEN_TEXT = 'implannotator-live-start';
const MARKER_CLOSE_TEXT = 'implannotator-live-end';
const IGNORE_MARKER_OPEN = '# implannotator-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# implannotator-live-ignore-end';
...
export const LIVE_IGNORE_PATTERNS = Object.freeze([
  ...
  '.implannotator/live/server.json',
  '.implannotator/live/sessions/',
  '.implannotator/live/previews/',
  '.implannotator/live/annotations/',
  '.implannotator/live/cache/',
  '.implannotator/live/manual-edit-apply-transaction.json',
  '.implannotator/live/manual-edit-events.jsonl',
  '.implannotator/live/manual-edit-evidence/',
  '.implannotator/live/pending-manual-edits.json',
  '.implannotator/live/deferred-svelte-component-accepts.json',
  '.implannotator-live.json',
  '.implannotator-live/',
  'node_modules/.implannotator-live/',
  'src/lib/implannotator/ImplannotatorLiveRoot.svelte',
  'src/lib/implannotator/__runtime.js',
  'src/lib/implannotator/[0-9a-f]*/',
]);
...
Reads configuration from .implannotator/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .implannotator/live/config.json exists and its content
...
function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*implannotator-live-start\s*-->[\s\S]*?<!--\s*implannotator-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*implannotator-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*implannotator-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
  for (const pat of patterns) {
    let changed = false;
...
// and stash the original `content` value in a `data-implannotator-csp-original`
...
const CSP_MARKER_ATTR = 'data-implannotator-csp-original';
