---
name: implannotator
description: Use for any frontend design, redesign, implementation, critique, audit, polish, UX, accessibility, responsive, performance, theming, typography, motion, interaction, onboarding, or design-system work that should be planned, human-approved, implemented, browser-verified, reviewed, and repaired through a durable workflow. Supports explicit /implannotator invocation and automatic selection for frontend work. Not for backend-only or non-UI tasks.
version: 0.1.0
---

Designs and iterates production-grade frontend interfaces through mandatory human approval, adaptive QA, Plannotator review, and bounded repair loops. Real working code, committed design choices, exceptional craft.

## Mandatory workflow

Read and follow [reference/implannotator-workflow.md](reference/implannotator-workflow.md) before any other workflow. Its approval gate overrides any command reference that would otherwise begin editing immediately.

Core sequence:

1. Call `implannotator_control` with `action: "start"` before project inspection.
2. Inspect read-only, load the selected command reference and register reference, then create a complete implementation plan.
3. Call `implannotator_control` with `action: "submit_plan"`. **Do not mutate project files unless it returns approval.**
4. Call `implannotator_control` with `action: "begin_implementation"`, then implement as the sole writer.
5. Run the adaptive QA profile in [reference/qa.md](reference/qa.md) and record the complete report with `action: "record_qa"`.
6. Open the initial review with `action: "review", stage: "initial"`.
7. Repair findings up to three times. Record each repair and rerun QA, but never open intermediate review tabs.
8. Open exactly one final review with `action: "review", stage: "final"` after QA passes or the third repair is complete.
9. Finish only after final approval or a warned, recorded user override.

If Plannotator is unavailable, follow [reference/approval.md](reference/approval.md). If the user rejects or closes a review, ask whether to revise, proceed anyway, or stop. Proceeding requires a second warning and must be recorded. Optional subagents may review read-only; only the main agent may edit.

Resolve every relative path in this skill against this skill directory. Never assume it is installed inside the target project.

## Setup

You MUST do these steps before proceeding:

1. Run `node <implannotator-skill-directory>/scripts/context.mjs` once per session. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, run `node <implannotator-skill-directory>/scripts/palette.mjs` to receive a brand seed color and composition guidance. This is the anchor for your primary brand color. Compose the rest of the palette (bg, surface, ink, accent, muted) around it per the script's instructions. Use OKLCH throughout. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.**
...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Setup has already run `context.mjs`; if that reported `NO_PRODUCT_MD` you are already in init (setup), so finish that and skip this. Otherwise run `node <implannotator-skill-directory>/scripts/context-signals.mjs` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**
   ...
   **If `scan.targets` is non-empty, run `node <implannotator-skill-directory>/scripts/detect.mjs --json <scan.targets joined by spaces>` once** (the bundled detector over local files: no network, no npx). `scan.via` tells you what they are: `git-changes` (the markup/style files in your dirty tree, the most relevant set), `source-dir` (e.g. `src`, `app`), `html`, or `root`. Fold the hits into your picks: many quality / contrast hits → `audit` or `polish`; a specific slop family → the matching command (gradient text or eyebrows → `quieter` / `typeset`, flat or gray palette → `colorize`, and so on). It's a real, current signal that beats guessing. If detect errors or the tree is large and slow, skip it and recommend the user run `audit` themselves; never block the suggestion on it.
...
Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke `$implannotator`.
...
**Pin** creates a standalone shortcut so `$<command>` invokes `$implannotator <command>` directly. **Unpin** removes it. The script writes to every harness directory present in the project.

```bash
node <implannotator-skill-directory>/scripts/pin.mjs <pin|unpin> <command>
```
...
`$implannotator hooks <on|off|status|ignore-rule|ignore-file|ignore-value|reset>` manages the design detector hook for this project. The hook auto-runs the detector after direct UI file edits and surfaces findings as system reminders. Full flow is in [reference/hooks.md](reference/hooks.md); load it when the user invokes `$implannotator hooks` with any argument.